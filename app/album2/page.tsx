...
export default function coba() {
  ...
  const fotoId = Number(searchParams.get('id'))

  const datafoto = {
    memori_in_bali : [{
        
        judul:"Memori In Bali", by:"Salsa",
        profile1:"/assets/homepage/profile/image 9-1.png", 
        profile2:"/assets/homepage/profile/image 9-2.png", 
        profile3:"/assets/homepage/profile/image 9-3.png",
  ...
  const album = datafoto.memori_in_bali[0]
  const activeFoto =
  album.g_kiri.find(f => f.id === fotoId) ||
  album.g_kanan.find(f => f.id === fotoId)

  if (!fotoId || !activeFoto) {
  return (
  ...
  const totalFoto =
    (album?.g_kiri?.length ?? 0) + (album?.g_kanan?.length ?? 0);
